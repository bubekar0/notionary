...
function vrify($getar){ // AJAX: verify Email(3 reasons): i4got, crack
   /* Send email clickable link.  A user provided an Email which we need to verify
      to change his forgotten password (i4got), or to create an account (crack).
   */
   $PARAM_MYURL = param( "myurl" );
   list($_PREQ, $_CKPW, $_AREQ, $_CKAC, $_VREQ, $_CKVR, $_CODE, $_IGNO, $_EVER) = pglot(
       ["_PREQ","_CKPW","_AREQ","_CKAC","_VREQ","_CKVR","_CODE","_IGNO","_EVER"]);
   $jason = json_decode( stripslashes( $getar ), true );
   $quien = mysqli_real_escape_string( lsql(), $jason['quien'] );      // originator (i4got,crack)
   $pword = password_hash( $jason['pword'], PASSWORD_DEFAULT );        // new pword "i4got" or "crack"
   $razon = mysqli_real_escape_string( lsql(), $jason['razon'] );      // reason to verify: i4got, crack
...
function oauth(){ // AJAX: create GP|TW|FB account IFF Agreement and no extant account
   ...
   if( !konto( "userID", "aauser", "user", "$_SESSION[uname]") ){ // Create account IFF non-extant
      sql( "insert into aauser (userID,user,createDate,password,ulang)
            values(LAST_INSERT_ID(),'$_SESSION[uname]',NOW(),'','$_SESSION[slang]')");

      $r = mysqli_fetch_assoc(
              sql( "select AUTO_INCREMENT from information_schema.tables where table_schema='notionary_db' and table_name='aauser'" ) );

      $uidno = $r['AUTO_INCREMENT'] - 1;
